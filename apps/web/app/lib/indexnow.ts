...
export const INDEXNOW_CATCH_UP_WINDOW_MS = 26 * 60 * 60 * 1000;

type FetchImplementation = typeof fetch;
type IndexNowArticle = Pick<ApiArticle, "slug" | "published_at" | "updated_at">;
...
export function recentlyChangedArticleUrls(
  articles: IndexNowArticle[],
  now = Date.now(),
  siteOrigin = DEFAULT_SITE_ORIGIN,
): string[] {
  const cutoff = now - INDEXNOW_CATCH_UP_WINDOW_MS;

  return articles
    .filter((article) => {
      const publishedAt = parseApiDate(article.published_at)?.getTime() || 0;
      const updatedAt = parseApiDate(article.updated_at)?.getTime() || 0;
      return Math.max(publishedAt, updatedAt) >= cutoff;
    })
    .map((article) => articleUrl(article.slug, siteOrigin));
}
