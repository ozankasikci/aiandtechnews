...
Tasks 5 through 12 currently provide the four public article reads, public category and author listings, compatible login, current-user, and logout endpoints, a pure Go port of the retained publishing policy, the authenticated dashboard content administration (dashboard article list/get/create/update/delete, category list/create/update/delete, and site settings get/put), the dashboard media library (media list/upload/delete plus static `/uploads/*` serving from a local directory), and the newsletter (signup, confirm and unsubscribe links, the edition archive, and the daily digest delivered through Resend). Migration task 14 adds cutover readiness: explicit production configuration guarded by a data marker, the `cmd/adopt` command that verifies and adopts the Node-created production database, a local smoke script, and the Mac mini runbooks in [`docs/cutover.md`](docs/cutover.md) and [`docs/rollback.md`](docs/rollback.md). The cutover ran on 2026-09-24: production is now the Go API on the Mac mini, run by per-user LaunchAgents from `~/aiandtechnews`; `docs/cutover.md` records it and holds the deploy and operations steps.
Access to the Mac mini (SSH, disk permissions, services, deploy gotchas): [docs/mac-mini-access.md](docs/mac-mini-access.md).
...
- **Production migrate:** with `APP_ENV=production`, `cmd/migrate` never creates the database or its directory and refuses a database without the migration ledger (checked on a read-only connection first, so a refused database is not touched), pointing to `cmd/adopt`; it only applies new migrations to an adopted database. Without this, a wrong `DATABASE_PATH` would yield a new, fully migrated, empty database that the API's schema check would accept.
...
- **`--apply`** writes `<DATABASE_PATH>.pre-adopt-<UTC timestamp>.db` with `VACUUM INTO` (refusing an existing file; it includes frames still in the `-wal` file) and checks it, then calls `migrate.Adopt`: in one `BEGIN IMMEDIATE` transaction it recounts every table (the counts must equal the backup's, so a concurrent writer stops it), verifies the schema again, creates the ledger, records the present migrations with their exact names and checksums, runs the pending ones (for a Node database: newsroom 3, 4 and 7), and, before `COMMIT`, requires the complete Go schema and row counts where no existing table disappeared, lost rows, or gained rows except `settings` (+2 at most, migration 3's `newsroom.*` rows). Recording 1, 2, 5, 6 and then calling `migrate.Run` would fail with `ErrHistoryGap`, hence the single transaction. `migrate.Run` afterwards is a no-op, and the result is verified against the reference again. Nothing is dropped, rewritten, or deleted; migration 3 adds its two `newsroom.*` settings rows. A database that is already adopted gets later migrations (such as 7) from `cmd/migrate` as usual.
...
`POST /api/newsroom/candidates/{id}/publish-now` queues a pending, queued, or
failed candidate due immediately and flags it (migration 7's
`candidate_publish_now` table, shown as `publish_now` on the candidate). The
publisher claims a flagged candidate ahead of the rest of the queue and
ignores the minimum gap for it, but still never while another candidate is
processing; the request also wakes the publisher loop instead of waiting for
the next tick. Other queued candidates keep their times. A trigger clears the
