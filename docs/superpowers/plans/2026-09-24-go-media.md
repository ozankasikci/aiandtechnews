...
3. **Upload read deadline of 2 minutes** (see below, change 3).
...
- **Server timeouts:** `httpserver.NewServer` has `ReadTimeout: 15s`, so a 5 MiB upload needs roughly 350 KB/s. Node has no such limit. Fine on a LAN; revisit if uploads over slow links fail.
