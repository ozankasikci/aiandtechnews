...
1. **Upload filter: declared type and extension must match.** The original name's extension (Node's `path.extname`, compared case-insensitively) must be `.jpg` or `.jpeg` for `image/jpeg`, `.png` for `image/png`, `.webp` for `image/webp`, and `.gif` for `image/gif`. Anything else, including no extension, `.png` as a whole name, `a.`, `p.png.html`, or a mismatch such as `p.png` declared `image/jpeg`, is rejected like a bad type: `500 {"error":"Only .jpg, .png, .webp, and .gif files are allowed"}` (status kept at 500 for Node parity), checked after the field check and before the size limit, leaving no file. The stored name still keeps the extension's original case. This closes the stored-XSS surface where `x.html` declared `image/png` was served as HTML from the API origin. Code: `acceptedImage` in `internal/media/http_library.go`.
