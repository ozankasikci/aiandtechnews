...
**Goal:** Port the Node dashboard media library to the Go API with exact behavior: `GET /api/dashboard/media`, `POST /api/dashboard/media/upload`, `DELETE /api/dashboard/media/:id`, and static serving of `/uploads/*`. Files stay on local disk by default (user decision: local disk with exact Node parity). An opt-in S3 backend for new uploads was added after review (see "Approved changes after review", item 4). At cutover the MacBook's `apps/server/uploads` directory is copied to the new host together with the database, so every existing `/uploads/...` URL stored in `articles.featured_image`, `authors.avatar`, and `media.url` must keep resolving. This is migration task 11 in `docs/plans/2026-09-20-go-api-migration.md`.
...
5. **`UPLOADS_DIR` overlap checks (code review).** `Config.Validate` rejects an `UPLOADS_DIR` that contains `DATABASE_PATH` (the database's directory or any ancestor) in every mode, because the media library deletes files inside it by name; and, outside `APP_ENV=production`, one that equals, contains, or lies inside the Node MacBook uploads directory (`ErrProductionUploadsAlias`). Paths are compared canonically (symlinks resolved), so `/x/data` is not treated as inside `/x/dat`. Pinned by `TestValidateRejectsAnUploadsDirThatContainsTheDatabase` and `TestValidateRejectsOverlapWithTheProductionUploadsDirOutsideProduction`.

6. **Bounded S3 writes (code review).** Every `PutObject`, for media uploads (`S3Uploads.Save`) and for feature images (`Store.StoreWebP`), runs under its own 60s timeout (`putObject` in `internal/media/storage.go`), independent of the caller's context. Pinned by `TestS3UploadsBoundPutObjectWithItsOwnTimeout` and `TestStoreWebPBoundsPutObjectWithItsOwnTimeout`.

7. **Download write deadline (code review).** `GET`/`HEAD /uploads/*` sets a 2-minute write deadline (`media.DownloadWriteTimeout`, `http.ResponseController.SetWriteDeadline`) once a file has been opened and is about to be sent; 404s keep the server's 30s. A slow client can therefore hold a download connection for up to 2 minutes, which is acceptable for files of at most a few MiB. Pinned by `TestServeExtendsTheWriteDeadlineForDownloadsOnly`.
...
| Write timeout for `/uploads/*` downloads | none (Node has no server write timeout) | 2 minutes per download (server default is 30s); 404s keep 30s |
| Upload read timeout | none | 2 minutes on `POST /api/dashboard/media/upload` (server default is 15s) |
...
- **Moving legacy files to S3.** Local disk is the default backend (user decision); the S3 backend added after review (Approved change 4) is opt-in via `MEDIA_STORAGE=s3` and only receives new uploads. Migrating existing `/uploads` files and rewriting their URLs is out of scope.
...
+**Status (2026-09-24): Complete for the preserved migration slice via `docs/superpowers/plans/2026-09-24-go-media.md`: local-disk storage in `UPLOADS_DIR` with Node parity (user decision: local disk by default; an opt-in S3 backend for new uploads followed after review), migration 5 (Node's `media` table), static `/uploads/*` serving, and the Node media contracts replaying in `internal/app/media_contract_test.go`. This does not establish production or cutover readiness.**
+
