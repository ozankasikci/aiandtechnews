...
2. **Any original extension is kept, case included** (`evil.html` declared `image/png` is stored as `<hex>.html`; `A.Photo.JPEG` → `.JPEG`; `noext` → no extension; `.png` → no extension; `a.` → `.`). Node's static server then serves `<hex>.html` as `text/html; charset=UTF-8`; Go serves the same Content-Type. See Open question 1. [`TestUploadNamesAndMIMETypesMatchNode`, `TestStaticContentTypeFollowsTheStoredExtensionLikeNode`]
...
- **Content sniffing, extension allowlists, or `Content-Security-Policy` on `/uploads/*`** (Open question 1).
...
- **Stored XSS surface (pre-existing):** an editor can upload `x.html` declared as `image/png`, and `/uploads/<hex>.html` is served as `text/html` from the API origin. Kept for parity (Open question 1).
...
1. **Harden `/uploads/*` after cutover?** Options: store only image extensions (`.jpg/.jpeg/.png/.webp/.gif`) and derive the extension from the MIME type; sniff content; or add `X-Content-Type-Options: nosniff` and `Content-Security-Policy: sandbox` to static responses. Each is a reviewed contract change; this plan keeps Node's behavior.
2. **Store original names as proper UTF-8?** Node stores `cafÃ©.webp` for `café.webp` (busboy's latin1 default). Go reproduces it for parity; fixing it only changes the display name in the dashboard.
3. **Return 400/413 instead of 500 for rejected uploads?** Node answers 500 for invalid types and oversize files; the dashboard shows the message either way.
4. **Production `UPLOADS_DIR` path on the new host** (for example `/var/lib/technews/uploads`): this plan only requires it to be explicit and absolute.
