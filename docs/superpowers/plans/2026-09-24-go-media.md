...
2. **`X-Content-Type-Options: nosniff` on every `/uploads/*` response,** including 404s and legacy files Node accepted with other extensions. Code: `Uploads.serve` in `internal/media/static.go`.
...
- Check the copied directory for non-image extensions (Node accepted any): they stay served with `nosniff` but should be reviewed or removed.
